...
router.get("/receipts/:id", requireSchoolAuth, async (req, res, next) => { try {\n    if (!validId(req.params.id)) return res.status(400).json({ success: false, message: "Invalid receipt id" });\n    const payment = await FeePayment.findById(req.params.id).populate("pupil", "name email parentPhone").lean();\n    if (!payment) return res.status(404).json({ success: false, message: "Payment receipt not found" });\n    const user = req.schoolUser; const allowed = user.role === "admin" || (user.role === "pupil" && String(user._id) === String(payment.pupil?._id)) || (user.role === "parent" && Array.isArray(user.children) && user.children.some(x => String(x) === String(payment.pupil?._id))) || (user.role === "sponsor" && Array.isArray(user.sponsoredPupils) && user.sponsoredPupils.some(x => String(x) === String(payment.pupil?._id)));\n    if (!allowed) return res.status(403).json({ success: false, message: "You do not have access to this receipt." });\n    return res.json({ success: true, receipt: { receiptNumber: payment.reference || "PAY-" + payment._id.toString().slice(-8).toUpperCase(), schoolName: "Angels Home Education Centre", payment } });\n  } catch (error) { next(error); }\n});\n\nrouter.get("/payments", requireSchoolAuth, admin, async (req, res, next) => {
