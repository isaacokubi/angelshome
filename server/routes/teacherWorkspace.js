...
function nairobiDayRange(date = new Date()) {
  ...
  return { start: new Date(`${day}T00:00:00+03:00`), end: new Date(`${day}T00:00:00+03:00`).getTime() + 86400000 };
}
...
router.get("/", requireSchoolAuth, async (req, res, next) => {
  try {
    ...
    const validTimetable = timetable.filter((row) => row.schoolClass?.isActive !== false && row.subject?.isActive !== false && row.teacher?.isActive !== false);
    const classIds = [...new Set(validTimetable.map((row) => row.schoolClass?._id?.toString()).filter(validId))].map((id) => new mongoose.Types.ObjectId(id));
    ...
    const [classTeacherClasses, learning, pupils, attendanceRows, results, notifications] = await Promise.all([
      ...
      classIds.length ? Attendance.find({ schoolClass: { $in: classIds }, date: { $gte: start, $lt: new Date(end) } }).select("pupil schoolClass date status note").lean() : [],
    ...
    const mergedClassIds = [...new Set([...classIds.map(String), ...classTeacherClasses.map((item) => String(item._id)))];
    ...
    return res.json({
      ...
      classes: [...new Map([...classTeacherClasses, ...validTimetable.map((row) => row.schoolClass)].filter(Boolean).map((item) => [String(item._id), item])).values()],
      pupils,
      learning,
      ...
      results,
      notifications,
      stats: { classes: mergedClassIds.length, pupils: pupils.length, subjects: subjectIds.length, lessonsToday: dayLessons.length, learningRecords: learning.length, averageProgress },
    });
