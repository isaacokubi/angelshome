...
export default function PortalDashboard() {
  ...
  const load = useCallback((showLoading = false) => {
    ...
    return Promise.all([portalApi.dashboard(), portalApi.metrics()]).then(([result, metricResult]) => { setData(result); setMetrics(metricResult); setError(""); }).catch((err) => setError(err.message)).finally(() => setLoading(false));
  }, []);
  useEffect(() => {
    let active = true;
    load(true).catch(() => {});
  ...
  return <PortalShell role={role}>
    ...
    {data && role !== "admin" && role !== "sponsor" && <StandardDashboard data={data} role={role} metrics={metrics} />}
  </PortalShell>;
}
...
function ParentDashboard({ data, metrics }) {
  ...
  return <>
    ...
    <SharedSchoolSnapshot data={data} role="parent" metrics={metrics} />
