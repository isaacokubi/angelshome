...
const roleActions = {
  admin: [["Manage pupils", "/portal/admin/pupils"], ["Manage teachers", "/portal/admin/teachers"], ["Manage parents", "/portal/admin/parents"], ["Manage sponsors", "/portal/admin/sponsors"]],
  teacher: [["My classes", "/portal/teacher/classes"], ["School notifications", "/portal/notifications"]],
  pupil: [["My learning", "/portal/pupil/learning"], ["School notifications", "/portal/notifications"]],
  parent: [["School updates", "/portal/notifications"], ["My children", "/portal/parent#children"]],
  sponsor: [["My impact", "/portal/sponsor/impact"], ["School notifications", "/portal/notifications"]],
};
...
export default function SharedSchoolSnapshot({ data, role }) {
  ...
  return <>
    ...
    <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{actions.map(([label, href]) => <Link key={href} to={href} className="rounded-2xl border border-slate-200 bg-white p-4 text-sm font-bold text-blue-950 shadow-sm transition hover:-translate-y-0.5 hover:border-blue-200 hover:bg-blue-50">{label}<span className="ml-2">→</span></Link>)}</section>
  </>;
}
