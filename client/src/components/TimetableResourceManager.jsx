...
export default function TimetableResourceManager({ locked = false, onChanged }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      setError(e.message || "Unable to load classes and subjects.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const remove = async (type, item) => {
    ...
    setError("");
    try {
      await apiRequest(`/smis/timetable/${type}s/${item._id}`, { method: "DELETE" });
      await load();
      onChanged?.();
    } catch (e) {
      setError(e.message || `Unable to delete ${resource}.`);
    } finally {
      setBusy("");
    }
  };
  ...
  return <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
    <div className="mb-4 flex flex-col justify-between gap-2 md:flex-row md:items-center">
      <div><h3 className="text-base font-black text-blue-950">Manage classes & subjects</h3><p className="text-xs text-slate-500">Remove incorrect resources before building lesson plans. Deletion is disabled while the timetable is locked.</p></div>
      <button type="button" onClick={() => void load()} disabled={loading || Boolean(busy)} className="rounded-lg border bg-white px-3 py-2 text-xs font-bold">{loading ? "Refreshing…" : "Refresh"}</button>
    </div>
